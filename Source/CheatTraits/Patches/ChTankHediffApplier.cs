...
namespace CheatTraits.Patches
{
    internal static class ChTankHediffApplier
    {
        ...
        private static HediffDef Dampener =>
            DefDatabase<HediffDef>.GetNamedSilentFail(DampenerHediffDefName);
        ...
        public static void TickPawn(Pawn p)
        {
            ...
            bool hasTank = CheatTraitsUtils.HasTrait(p, CheatTraitsNames.TankTrait);
            var existing = p.health.hediffSet.GetFirstHediffOfDef(dampener);

            if (hasTank)
            {
                if (existing == null)
                    p.health.AddHediff(dampener);
            }
            else
            {
                if (existing != null)
