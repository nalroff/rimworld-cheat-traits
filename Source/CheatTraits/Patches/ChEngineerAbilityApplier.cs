...
{
    internal static class ChEngineerAbilityApplier
    {
        ...
        private static AbilityDef? Def
        {
            get
            {
                if (!cachedResolved)
                {
                    cachedDef = DefDatabase<AbilityDef>.GetNamedSilentFail("ChRetrofit");
                    cachedResolved = true;
                }
                return cachedDef;
            }
        ...
        public static void TickPawn(Pawn p)
        {
            ...
            bool hasTrait = CheatTraitsUtils.HasTrait(p, CheatTraitsNames.EngineerTrait);
