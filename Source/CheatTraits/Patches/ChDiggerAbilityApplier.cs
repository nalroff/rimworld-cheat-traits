...
{
    internal static class ChDiggerAbilityApplier
    {
        ...
        private static AbilityDef? Def
        {
            get
            {
                if (!cachedResolved)
                {
                    cachedDef = DefDatabase<AbilityDef>.GetNamedSilentFail("ChTunnel");
                    cachedResolved = true;
                }
                return cachedDef;
            }
        ...
        public static void TickPawn(Pawn p)
        {
            ...
            bool hasTrait = CheatTraitsUtils.HasTrait(p, CheatTraitsNames.DiggerTrait);
