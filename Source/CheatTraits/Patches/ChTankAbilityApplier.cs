...
{
    internal static class ChTankAbilityApplier
    {
        ...
        private static AbilityDef? Def
        {
            get
            {
                if (!cachedResolved)
                {
                    cachedDef = DefDatabase<AbilityDef>.GetNamedSilentFail("ChIronWall");
                    cachedResolved = true;
                }
                return cachedDef;
            }
        ...
        public static void TickPawn(Pawn p)
        {
            ...
            bool hasTrait = CheatTraitsUtils.HasTrait(p, CheatTraitsNames.TankTrait);
