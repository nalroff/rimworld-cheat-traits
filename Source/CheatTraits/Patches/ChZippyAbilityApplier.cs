...
{
    internal static class ChZippyAbilityApplier
    {
        ...
        private static AbilityDef? Def
        {
            get
            {
                if (!cachedResolved)
                {
                    cachedDef = DefDatabase<AbilityDef>.GetNamedSilentFail("ChBlink");
                    cachedResolved = true;
                }
                return cachedDef;
            }
        ...
        public static void TickPawn(Pawn p)
        {
            ...
            bool hasTrait = CheatTraitsUtils.HasTrait(p, CheatTraitsNames.ZippyTrait);
